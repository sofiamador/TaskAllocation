...
public class URLConnectionReader {
	...
	public double getDemoDistance(Location from, Location to) {
		double lat = 100*(Math.abs(from.getLat()-to.getLat()));
		double lng = 100*(Math.abs(from.getLng()-to.getLng()));
		return Math.sqrt(Math.pow(lat,2)+Math.pow(lng,2));
	}
	public double getDemoDistance2(Location from, Location to) {
		double lat = 0.1*(Math.abs(from.getLat()-to.getLat()));
		double lng =0.1*(Math.abs(from.getLng()-to.getLng()));
		return Math.sqrt(Math.pow(lat,2)+Math.pow(lng,2));
	}
